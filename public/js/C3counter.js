	function C3Counter(id, opt) {
			...
			this.options = {
				...
				digitSlide : true,
				digitSlideTime : 200,
				digitImageHeight : 704,
				digitAnimationHeight : 64, 
				timerEnd: function(){},
				image: "digits.png", 
				updateInterval : 1000,
				...
				digitWidth2: 10,
				digitHeight2: 27,
				digitImageHeight2 : 270,
				digitAnimationHeight2 : 27				
			...
			for (dc=2; dc<8; dc++) {
				...
				$("#"+id).append("<div class='digit_wr' id='digit"+dc+"'  style='position:relative;float:left;width:"+this.options.digitWidth2+"px;height:"+this.options.digitHeight2+"px;overflow:hidden;'><div class='digit' id='digit-bg"+dc+"' style='position:absolute; top:-"+digits[dc].digit*this.options.digitAnimationHeight2+"px; width:"+this.options.digitWidth2+"px; height:"+this.options.digitImageHeight2+"px; '></div></div>");
			...
			this.animateDigits = function() {
				...
				for (var dc=2; dc<8; dc++) {
					...
					if (digits[no].digit == 0) $("#digit-bg"+no).css("top", -this.options.digitImageHeight2+this.options.digitHeight2 + "px");
					if (digits[no].digit != digits[no].digitNext) {
						$("#digit-bg"+no).animate( { "top" : -digits[no].digitNext*options.digitHeight2+"px"}, options.digitSlideTime);
						digits[no].digit = digits[no].digitNext;
					}
